 ...
 **************************************************************************************************/
long mydns_rr_count(SQL *sqlConn) {
 ...
 **************************************************************************************************/
void mydns_set_rr_table_name(char *name) {
	if (!name)
		strncpy(mydns_rr_table_name, MYDNS_RR_TABLE,
				sizeof(mydns_rr_table_name) - 1);
	else
 ...
 **************************************************************************************************/
void mydns_set_rr_where_clause(char *where) {
...
/**************************************************************************************************
 MYDNS_RR_GET_TYPE
 **************************************************************************************************/
inline dns_qtype_t mydns_rr_get_type(char *type) {
	register char *c;
...
#if ALIAS_ENABLED
		if (type[1] == 'L' && type[2] == 'I' && type[3] == 'A' && type[4] == 'S'
				&& !type[5])
	...
	case 'C':
		if (type[1] == 'N' && type[2] == 'A' && type[3] == 'M' && type[4] == 'E'
				&& !type[5])
			return DNS_QTYPE_CNAME;
		break;

	case 'D':
		if (type[1] == 'N' && type[2] == 'S' && type[3] == 'K' && type[4] == 'E'
				&& type[5] == 'Y' && !type[6])
	...
	case 'H':
		if (type[1] == 'I' && type[2] == 'N' && type[3] == 'F' && type[4] == 'O'
				&& !type[5])
	...
	case 'R':
		if (type[1] == 'P' && !type[2])
			return DNS_QTYPE_RP;
		if (type[1] == 'R' && type[2] == 'S' && type[3] == 'I' && type[4] == 'G'
				&& !type[5])
	...
	case 'S':
		...
		if (type[1] == 'S' && type[2] == 'H' && type[3] == 'F' && type[4] == 'P'
				&& !type[5])
 ...
 **************************************************************************************************/
static inline void mydns_rr_parse_rp(SQL_ROW row, const char *origin,
		MYDNS_RR *rr) {
	char *c;
 ...
 **************************************************************************************************/
static inline void mydns_rr_parse_srv(SQL_ROW row, const char *origin,
		MYDNS_RR *rr) {
 ...
 **************************************************************************************************/
static inline void mydns_rr_parse_sshfp(SQL_ROW row, const char *origin,
		MYDNS_RR *rr) {
...
/**************************************************************************************************
 MYDNS_RR_PARSE_NSEC
 NSEC records contain ...
 **************************************************************************************************/
static inline void mydns_rr_parse_nsec(SQL_ROW row, const char *origin,
		MYDNS_RR *rr) {
 ...
 **************************************************************************************************/
static inline int mydns_rr_parse_naptr(SQL_ROW row, const char *origin,
		MYDNS_RR *rr) {
 ...
 **************************************************************************************************/
inline MYDNS_RR *mydns_rr_parse(SQL_ROW row, const char *origin, dns_qtype_t type) {
	MYDNS_RR *rr;
	...
	if ((rr = (MYDNS_RR *) calloc(1, sizeof(MYDNS_RR)))) {
		...
		if (rr->type == DNS_QTYPE_ALIAS
				&& (type == DNS_QTYPE_A || type == DNS_QTYPE_AAAA
						|| type == DNS_QTYPE_ANY)) {
			rr->type = type;
			rr->alias = 1;
		} else
...
/**************************************************************************************************
 ...
 in the RRset.
 **************************************************************************************************/
MYDNS_RR *mydns_rr_dup(MYDNS_RR *start, int recurse) {
	...
	for (s = start; s; s = tmp) {
		...
		if (rr->type == DNS_QTYPE_NAPTR) {
			...
			strncpy(rr->naptr_service, s->naptr_service,
					sizeof(rr->naptr_service) - 1);
			strncpy(rr->naptr_regex, s->naptr_regex, sizeof(rr->naptr_regex) - 1);
			strncpy(rr->naptr_replacement, s->naptr_replacement,
					sizeof(rr->naptr_replacement) - 1);
		}
...
/**************************************************************************************************
 MYDNS_RR_SIZE
 **************************************************************************************************/
inline size_t mydns_rr_size(MYDNS_RR *first) {
	register MYDNS_RR *p;
 ...
 **************************************************************************************************/
inline void _mydns_rr_free(MYDNS_RR *first) {
 ...
 **************************************************************************************************/
int mydns_rr_load(SQL *sqlConn, MYDNS_RR **rptr, uint32_t zone,
		dns_qtype_t type, char *name, char *origin) {
	MYDNS_RR *first = NULL, *last = NULL;
	size_t querylen;
	char *wheretype, query[DNS_QUERYBUFSIZ], namequery[DNS_MAXNAMELEN
			+ DNS_MAXNAMELEN + DNS_MAXNAMELEN + 25] = "";
	register char *c, *cp;
	SQL_RES *res;
	SQL_ROW row;
...
#if ALIAS_ENABLED
	case DNS_QTYPE_A:
		wheretype = " AND (type='A' OR type='ALIAS')";
		break;
	case DNS_QTYPE_AAAA:
		wheretype = " AND (type='AAAA' OR type='ALIAS')";
		break;
#else
	...
	case DNS_QTYPE_NS:
		...
		break;
	case DNS_QTYPE_PTR:
		wheretype = " AND type='PTR'";
		break;
	case DNS_QTYPE_SOA:
		wheretype = " AND type='SOA'";
		break;
	case DNS_QTYPE_SPF:
		wheretype = " AND type='SPF'";
		break;
	case DNS_QTYPE_SRV:
		wheretype = " AND type='SRV'";
		break;
	case DNS_QTYPE_SSHFP:
		wheretype = " AND type='SSHFP'";
		break;
	case DNS_QTYPE_TXT:
		wheretype = " AND type='TXT'";
		break;
	case DNS_QTYPE_ANY:
		wheretype = "";
		break;
	...
	if (name) {
		if (origin) {
			if (!name[0])
				snprintf(namequery, sizeof(namequery), "(name='' OR name='%s')",
						origin);
			else {
	...
	querylen = snprintf(query, sizeof(query),
			...
			"zone=%u%s"
			"%s%s%s%s", (mydns_rr_use_active ? ",active" : ""),
			mydns_rr_table_name, zone, wheretype, (namequery[0]) ? " AND " : "",
			namequery, (mydns_rr_where_clause) ? " AND " : "",
	...
	while ((row = sql_getrow(res))) {
		...
		/* Obey "active" column */
		if (mydns_rr_use_active && row[MYDNS_RR_NUMFIELDS]
				&& !GETBOOL(row[MYDNS_RR_NUMFIELDS]))
			continue;
