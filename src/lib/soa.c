 ...
 **************************************************************************************************/
long mydns_soa_count(SQL *sqlConn) {
 ...
 **************************************************************************************************/
void mydns_set_soa_table_name(char *name) {
	if (!name)
		strncpy(mydns_soa_table_name, MYDNS_SOA_TABLE,
				sizeof(mydns_soa_table_name) - 1);
	else
 ...
 **************************************************************************************************/
void mydns_set_soa_where_clause(char *where) {
...
inline
#endif
MYDNS_SOA *mydns_soa_parse(SQL_ROW row) {
	MYDNS_SOA *rv;
	int len;
...
/**************************************************************************************************
 ...
 in the list.
 **************************************************************************************************/
MYDNS_SOA *mydns_soa_dup(MYDNS_SOA *start, int recurse) {
...
inline
#endif
size_t mydns_soa_size(MYDNS_SOA *first) {
...
inline
#endif
void _mydns_soa_free(MYDNS_SOA *first) {
 ...
 **************************************************************************************************/
int mydns_soa_load(SQL *sqlConn, MYDNS_SOA **rptr, char *origin) {
	...
	if (mydns_soa_where_clause)
		querylen = snprintf(query, sizeof(query),
				...
				(mydns_soa_use_active ? ",active" : ""), mydns_soa_table_name,
				origin, mydns_soa_where_clause);
	else
		querylen = snprintf(query, sizeof(query),
	...
	while ((row = sql_getrow(res))) {
		...
		if (mydns_soa_use_active && row[MYDNS_SOA_NUMFIELDS]
				&& !GETBOOL(row[MYDNS_SOA_NUMFIELDS]))
			continue;
