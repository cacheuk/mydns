 ...
 **************************************************************************************************/
int new_task(TASK *t, char *data, size_t len) {
	...
	if (t->hdr.qr) {
		formerr(t, DNS_RCODE_FORMERR, ERR_RESPONSE_BIT_SET,
	...
	if (t->protocol == SOCK_STREAM && !tcp_enabled
			&& (t->qtype != DNS_QTYPE_AXFR || !axfr_enabled))
	...
	if (t->qtype == DNS_QTYPE_AXFR
			&& (!axfr_enabled || t->protocol != SOCK_STREAM))
		...
		} else /* Parent */
		{
			...
			for (errct = 0; errct < 5; errct++) {
				if (read(pfd[0], &buf, 4) != 2)
					Warn("%s (%d of 5)", _("error reading startup notification"),
							errct + 1);
				else
					break;
			}
...
char *desctask(TASK *t) {
	...
	snprintf(desc, sizeof(desc), "%s: %s %s", clientaddr(t),
 ...
 **************************************************************************************************/
TASK *_task_init(taskstat_t status, /* Initial status */
 ...
 **************************************************************************************************/
void _task_free(TASK *t, const char *file, int line) {
	if (!t)
		return;
	...
	if (answer_then_quit
			&& (Status.udp_requests + Status.tcp_requests) >= answer_then_quit)
...
/**************************************************************************************************
 TASK_OUTPUT_INFO
 **************************************************************************************************/
void task_output_info(TASK *t, char *update_desc) {
	...
	Verbose(
			...
			datebuf, tv.tv_usec,
#endif
			t->internal_id, t->id, t->protocol == SOCK_STREAM ? "TCP" : "UDP",
			clientaddr(t), mydns_class_str(t->qclass), mydns_qtype_str(t->qtype),
			t->qname, mydns_rcode_str(t->hdr.rcode), err_reason_str(t, t->reason),
			t->qdcount, t->an.size, t->ns.size, t->ar.size,
			(t->reply_from_cache ? "Y" : "N"), mydns_opcode_str(t->hdr.opcode),
 ...
 **************************************************************************************************/
void task_process(register TASK *t) {
	int rv;
	...
	if (t->status == NEED_READ) {
		...
		default:
			Warnx("%s: %d: %s", desctask(t), t->protocol,
					_("unknown/unsupported protocol"));
			return dequeue(Tasks, t);
		}
	}
