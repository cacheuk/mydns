 ...
 **************************************************************************************************/
static void addrlist_add(int family, void *address, int port) {
	ADDRLIST *A;
	...
	for (A = FirstAddr; A; A = A->next)
		...
		case AF_INET:
			if ((A->port == port)
					&& !memcmp(&addr4, &A->addr4, sizeof(struct in_addr)))
				return;
			break;
#if HAVE_IPV6
		case AF_INET6:
			if ((A->port == port)
					&& !memcmp(&addr6, &A->addr6, sizeof(struct in6_addr)))
				return;
			break;
#endif
		}
	...
	case AF_INET:
		memcpy(&A->addr4, &addr4, sizeof(struct in_addr));
		break;
#if HAVE_IPV6
	case AF_INET6:
		memcpy(&A->addr6, &addr6, sizeof(struct in6_addr));
		break;
#endif
	}
	A->port = port;
...
/**************************************************************************************************
 IP6_EXTRA
 **************************************************************************************************/
char *ip6_extra(const struct in6_addr *a) {
	...
	*b = '\0';
	...
	if (IN6_IS_ADDR_UNSPECIFIED(a))
		b += snprintf(b, sizeof(buf) - (b - buf), "UNSPECIFIED ");
	if (IN6_IS_ADDR_LOOPBACK(a))
		b += snprintf(b, sizeof(buf) - (b - buf), "LOOPBACK ");
	if (IN6_IS_ADDR_MULTICAST(a))
		b += snprintf(b, sizeof(buf) - (b - buf), "MULTICAST ");
	if (IN6_IS_ADDR_LINKLOCAL(a))
		b += snprintf(b, sizeof(buf) - (b - buf), "LINKLOCAL ");
	if (IN6_IS_ADDR_SITELOCAL(a))
		b += snprintf(b, sizeof(buf) - (b - buf), "SITELOCAL ");
	if (IN6_IS_ADDR_V4MAPPED(a))
		b += snprintf(b, sizeof(buf) - (b - buf), "V4MAPPED ");
	if (IN6_IS_ADDR_V4COMPAT(a))
		b += snprintf(b, sizeof(buf) - (b - buf), "V4COMPAT ");

	if (IN6_IS_ADDR_MULTICAST(a)) {
		if (IN6_IS_ADDR_MC_NODELOCAL(a))
			b += snprintf(b, sizeof(buf) - (b - buf), "MC_NODELOCAL ");
		if (IN6_IS_ADDR_MC_LINKLOCAL(a))
			b += snprintf(b, sizeof(buf) - (b - buf), "MC_LINKLOCAL ");
		if (IN6_IS_ADDR_MC_SITELOCAL(a))
			b += snprintf(b, sizeof(buf) - (b - buf), "MC_SITELOCAL ");
		if (IN6_IS_ADDR_MC_ORGLOCAL(a))
			b += snprintf(b, sizeof(buf) - (b - buf), "MC_ORGLOCAL ");
		if (IN6_IS_ADDR_MC_GLOBAL(a))
			b += snprintf(b, sizeof(buf) - (b - buf), "MC_GLOBAL ");
	}
...
#ifdef __linux__
#if HAVE_IPV6
static void linux_load_ip6(int port) {
	FILE *fp;
	char line[512];
	...
	if ((fp = fopen("/proc/net/if_inet6", "r"))) {
		while (fgets(line, sizeof(line), fp)) {
			...
			if (!(hex = strsep(&l, " ")))
				continue;
			if (!(device_number = strsep(&l, " ")))
				continue;
			if (!(prefix_length = strsep(&l, " ")))
				continue;
			if (!(scope_value = strsep(&l, " ")))
				continue;
			if (!(if_flags = strsep(&l, " ")))
				continue;
			strtrim(l);
...
#if DEBUG_ENABLED && DEBUG_LISTEN
			{
				...
				Debug(
						"IPv6 %s: device_number=%u prefix_length=%u scope_value=%u if_flags=%u",
						buf, dn, pl, sv, f);
			}
#endif
 ...
 **************************************************************************************************/
static ADDRLIST *addrlist_load(int port) {
	struct ifconf ifc;
	struct ifreq *ifr;
	...
	for (n = 0; n < ifc.ifc_len;) /* Scan interfaces */
	{
		...
		case AF_INET:
			memcpy(&addr4, &((struct sockaddr_in *) &ifr->ifr_addr)->sin_addr,
					sizeof(struct in_addr));
			addrlist_add(AF_INET, &addr4, port);
			break;
#if HAVE_IPV6
		case AF_INET6:
			memcpy(&addr6, &((struct sockaddr_in6 *) &ifr->ifr_addr)->sin6_addr,
...
/**************************************************************************************************
 ADDRLIST_FREE
 **************************************************************************************************/
void addrlist_free(ADDRLIST *Addresses) {
	ADDRLIST *A, *tmp;
...
/**************************************************************************************************
 ...
 the addresses.
 **************************************************************************************************/
static ADDRLIST *get_opt_addrlist(ADDRLIST *Addresses, const char *opt, int default_port,
		char *desc) {
	...
	for (cc = oc; (a = strsep(&cc, CONF_FS_STR));) /* Add each address */
	{
		...
		if (family == AF_INET) /* Add regular IPv4 address */
		{
			if (inet_pton(AF_INET, addr, &addr4) <= 0)
				Warnx("%s: `%s' %s: %s", addr, desc, _("address"),
 ...
 **************************************************************************************************/
static int ipv4_listener(struct sockaddr_in *sa, int protocol) {
	int fd, opt = 1;

	if ((fd = socket(AF_INET, protocol,
			(protocol == SOCK_STREAM) ? IPPROTO_TCP : IPPROTO_UDP)) < 0)
		Err("socket");
 ...
 **************************************************************************************************/
static int ipv6_listener(struct sockaddr_in6 *sa, int protocol) {
	int fd, opt = 1;

	if ((fd = socket(AF_INET6, protocol,
			(protocol == SOCK_STREAM) ? IPPROTO_TCP : IPPROTO_UDP)) < 0)
		Err("socket");
...
void create_listeners(void) {
	...
	Listen = get_opt_addrlist(Addresses, conf_get(&Conf, "listen", NULL), port,
			"listen");
	NoListen = get_opt_addrlist(Addresses, conf_get(&Conf, "no-listen", NULL),
			port, "no-listen");
...
static void server_greeting(void) {
	...
	g += sprintf(g, "%s %s %s %.24s", progname, PACKAGE_VERSION, _("started"),
			ctime(&now));
	...
	total += num_udp6_fd;
#endif
	g += sprintf(g, " (%s %d %s)", _("listening on"), total,
