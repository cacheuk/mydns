...
/**************************************************************************************************
 ACCEPT_TCP_QUERY
 **************************************************************************************************/
int accept_tcp_query(int fd, int family) {
 ...
 **************************************************************************************************/
static int read_tcp_length(TASK *t) {
	int rv;
	char len[2];
	...
	if ((t->len = ((len[0] << 8) | (len[1]))) < DNS_HEADERSIZE)
		return Warnx("%s: %s (%zu octet%s)", clientaddr(t),
				_("TCP message too short"), t->len, S(t->len));
	if (t->len > DNS_MAXPACKETLEN_TCP)
		return Warnx("%s: %s (%zu octet%s)", clientaddr(t),
				_("TCP message too long"), t->len, S(t->len));
 ...
 **************************************************************************************************/
static int write_tcp_length(TASK *t) {
	char len[2], *l;
	int rv;
