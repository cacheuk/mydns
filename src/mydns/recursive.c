...
int recursive_fwd(TASK *t) {
...
#endif
	{
		if ((t->recursive_fd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
			Warn("%s: %s", recursive_fwd_server,
					_("error creating socket for recursive forwarding"));
			return dnserror(t, DNS_RCODE_SERVFAIL, ERR_FWD_RECURSIVE);
		}
		fcntl(t->recursive_fd, F_SETFL,
				fcntl(t->recursive_fd, F_GETFL, 0) | O_NONBLOCK);
		if ((rv = connect(t->recursive_fd,
				(const struct sockaddr *) &recursive_sa, sizeof(struct sockaddr_in)))
				< 0) {
			Warn("%s: %s", recursive_fwd_server,
					_("error connecting to recursive forwarder"));
			return dnserror(t, DNS_RCODE_SERVFAIL, ERR_FWD_RECURSIVE);
		}
#if DEBUG_ENABLED && DEBUG_RECURSIVE
 ...
 **************************************************************************************************/
int recursive_fwd_connect(TASK *t) {
#if DEBUG_ENABLED && DEBUG_RECURSIVE
 ...
 **************************************************************************************************/
int recursive_fwd_write(TASK *t) {
	...
	if ((rv = sendto(t->recursive_fd, query, querylen, 0,
			(struct sockaddr *) &recursive_sa, sizeof(struct sockaddr_in)))
			!= querylen) {
		if (errno == EAGAIN) {
#if DEBUG_ENABLED && DEBUG_RECURSIVE
		...
		if (rv < 0)
			...
			Warnx("%s: %s", recursive_fwd_server,
					_("short count sending quersion to recursive forwarder"));
		return dnserror(t, DNS_RCODE_SERVFAIL, ERR_FWD_RECURSIVE);
	}
...
int recursive_fwd_read(TASK *t) {
	...
	if ((replylen = recvfrom(t->recursive_fd, &reply, sizeof(reply), 0,
		...
		Warn("%s: %s", recursive_fwd_server,
				_("error reading reply from recursive forwarder"));
		return dnserror(t, DNS_RCODE_SERVFAIL, ERR_FWD_RECURSIVE);
	}
	if (!replylen) {
		Warnx("%s: %s", recursive_fwd_server,
				_("no reply from recursive forwarder"));
		return dnserror(t, DNS_RCODE_SERVFAIL, ERR_FWD_RECURSIVE);
	}
