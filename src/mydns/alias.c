 ...
 **************************************************************************************************/
MYDNS_RR *find_alias(TASK *t, char *fqdn) {
 ...
 **************************************************************************************************/
int alias_recurse(TASK *t, datasection_t section, char *fqdn, MYDNS_SOA *soa,
		char *label, MYDNS_RR *alias) {
		...
		if ((rr = find_alias(t, name))) {
			...
			for (n = 0; n < depth; n++)
				if (aliases[n] == rr->id) {
					...
					Verbose("%s: %s: %s (depth %d)", desctask(t),
							_("ALIAS loop detected"), fqdn, depth);
					mydns_rr_free(rr);
					return (0);
				}
			aliases[depth] = rr->id;
			...
			Verbose("%s: %s: %s -> %s", desctask(t), _("ALIAS chain is broken"),
					fqdn, name);
			return (0);
		}
	}
	Verbose("%s: %s: %s -> %s (depth %d)", desctask(t),
			_("max ALIAS depth exceeded"), fqdn, alias->data, depth);
	return (0);
}
/*--- alias_recurse() ---------------------------------------------------------------------------*/
