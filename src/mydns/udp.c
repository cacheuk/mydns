 ...
 **************************************************************************************************/
int read_udp_query(int fd, int family) {
	...
	if (family == AF_INET6) {
		...
		if ((len = recvfrom(fd, &in, sizeof(in), 0, (struct sockaddr *) &addr6,
				&addrlen)) <= 0) {
			if (len == 0)
				return (-1);
			if (errno == EAGAIN)
...
#endif
	{
		...
		memset(&in, 0, sizeof(in));
		if ((len = recvfrom(fd, &in, sizeof(in), 0, (struct sockaddr *) &addr4,
				&addrlen)) <= 0) {
			if (len == 0)
				return (-1);
			if (errno == EAGAIN)
...
/**************************************************************************************************
 WRITE_UDP_REPLY
 **************************************************************************************************/
void write_udp_reply(TASK *t) {
	int rv;

#if HAVE_IPV6
	if (t->family == AF_INET6)
		rv = sendto(t->fd, t->reply, t->replylen, 0,
				(struct sockaddr *) &t->addr6, sizeof(t->addr6));
	else
#endif
		rv = sendto(t->fd, t->reply, t->replylen, 0,
				(struct sockaddr *) &t->addr4, sizeof(t->addr4));
