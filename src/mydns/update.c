 ...
 **************************************************************************************************/
static int update_transaction(TASK *t, const char *query) {
	if (sql_nrquery(sql, query, strlen(query)) != 0) {
		WarnSQL(sql, "%s: %s", desctask(t),
				_("error deleting all RRsets via DNS UPDATE"));
 ...
 **************************************************************************************************/
static int check_update(TASK *t, MYDNS_SOA *soa) {
	SQL_RES *res = NULL;
	SQL_ROW row;
	char ip[256];
	char query[512];
	...
	querylen = snprintf(query, sizeof(query),
			"SELECT update_acl FROM %s WHERE id=%u%s", mydns_soa_table_name,
			soa->id, mydns_rr_use_active ? " AND active=1" : "");
#if DEBUG_UPDATE_SQL
	...
	if (!(res = sql_query(sql, query, querylen)))
		ErrSQL(sql, "%s: %s", desctask(t),
				_("error loading DNS UPDATE access rules"));
...
/**************************************************************************************************
 UPDATE_RRDUMP
 **************************************************************************************************/
static void update_rrdump(TASK *t, char *section, int which, UQRR *rr) {
	...
	Debug(
			"%s: DNS UPDATE: >>> %s %d: name=[%s] type=%s class=%s ttl=%u rdlength=%u rdata=[%s]",
			desctask(t), section, which, rr->name, mydns_qtype_str(rr->type),
			mydns_class_str(rr->class), rr->ttl, rr->rdlength, buf);
}
/*--- update_rrdump() ---------------------------------------------------------------------------*/
#endif
 ...
 **************************************************************************************************/
static char *update_gobble_rr(TASK *t, MYDNS_SOA *soa, char *query, size_t querylen,
		char *current, UQRR *rr) {
	char *src = current;
 ...
 **************************************************************************************************/
static int parse_update_query(TASK *t, MYDNS_SOA *soa, UQ *q) {
 ...
 **************************************************************************************************/
static char *text_retrieve(char *src, char *end, char *data, size_t datalen,
		int one_word_only) {
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on error.
 **************************************************************************************************/
static int update_get_rr_data(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr,
		char *data, size_t datalen, uint32_t *aux) {
	char *src = rr->rdata;
	...
	*aux = 0;
	...
	case DNS_QTYPE_A:
		if (rr->rdlength != 4)
			return -1;
		snprintf(data, datalen, "%d.%d.%d.%d", rr->rdata[0], rr->rdata[1],
	...
	case DNS_QTYPE_HINFO: {
		...
		snprintf(data, datalen, "%s%s%s %s%s%s", data1sp ? "\"" : "", data1,
				data1sp ? "\"" : "", data2sp ? "\"" : "", data2,
				data2sp ? "\"" : "");
	}
		return 0;
 ...
 **************************************************************************************************/
static int update_in_zone(TASK *t, char *name, char *origin) {
 ...
 **************************************************************************************************/
static int update_zone_has_name(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	SQL_RES *res = NULL;
	SQL_ROW row;
	char query[512];
	size_t querylen;
	...
	if (!(res = sql_query(sql, query, querylen))) {
		WarnSQL(sql, "%s: %s", desctask(t),
				_("error searching name for DNS UPDATE"));
		return dnserror(t, DNS_RCODE_SERVFAIL, ERR_DB_ERROR);
	}
...
/**************************************************************************************************
 ...
 Returns 1 if the name exists, 0 if not, -1 on error.
 **************************************************************************************************/
static int update_zone_has_rrset(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	SQL_RES *res = NULL;
	SQL_ROW row;
	char query[512];
	size_t querylen;
	...
	if (!(res = sql_query(sql, query, querylen))) {
		WarnSQL(sql, "%s: %s", desctask(t),
				_("error searching name/type for DNS UPDATE"));
		return dnserror(t, DNS_RCODE_SERVFAIL, ERR_DB_ERROR);
	}
	if (sql_num_rows(res) > 0)
		found = 1;
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on error.
 **************************************************************************************************/
static int check_prerequisite(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	...
	if (rr->class == DNS_CLASS_ANY) {
		...
		for (n = 0, unique = 1; n < q->num_tmprr && unique; n++)
			if (q->tmprr[n]->type == rr->type
					&& !strcasecmp(q->tmprr[n]->name, rr->name)
					&& !strcasecmp(q->tmprr[n]->data, data)
					&& q->tmprr[n]->aux == aux)
				unique = 0;
		...
		if (unique) {
			...
			if (!(q->tmprr[q->num_tmprr] = malloc(sizeof(TMPRR))))
				Err(_("out of memory"));
			strncpy(q->tmprr[q->num_tmprr]->name, rr->name,
					sizeof(q->tmprr[q->num_tmprr]->name) - 1);
			q->tmprr[q->num_tmprr]->type = rr->type;
			strncpy(q->tmprr[q->num_tmprr]->data, data,
					sizeof(q->tmprr[q->num_tmprr]->data) - 1);
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on error.
 **************************************************************************************************/
static int check_tsig(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr, int *do_sign) {
	...
	if ((l = strlen(t->key->private)) < 10) { /* 10 chars = 8 octets = 64 bits */
		Verbose("%s: DNS UPDATE: key [%s] is too short to be secure", desctask(t),
				t->key->name);
		t->tsig_error = DNS_RCODE_BADKEY;
 ...
 **************************************************************************************************/
static inline int update_rrtype_ok(dns_qtype_t type) {
	switch (type) {
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on error.
 **************************************************************************************************/
static int prescan_update(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	/* Class must be ANY, NONE, or the same as the zone's class */
	if ((rr->class != DNS_CLASS_ANY) && (rr->class != DNS_CLASS_NONE)
			&& (rr->class != q->class)) {
#if DEBUG_ENABLED && DEBUG_UPDATE
	...
	if ((rr->class == DNS_CLASS_ANY)
			&& (!update_rrtype_ok(rr->type) && rr->type != DNS_QTYPE_ANY)) {
#if DEBUG_ENABLED && DEBUG_UPDATE
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on failure.
 **************************************************************************************************/
static int update_add_rr(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	static char data[DNS_MAXPACKETLEN_UDP];
	uint32_t aux;
	...
	if (!duplicate) {
		querylen = snprintf(query, sizeof(query),
				...
				mydns_rr_table_name, mydns_rr_use_active ? ",active" : "", soa->id,
				xname, mydns_qtype_str(rr->type), xdata, aux, rr->ttl,
				mydns_rr_use_active ? ",'" : "", mydns_rr_use_active ? "Y" : "",
				mydns_rr_use_active ? "'" : "");
#if DEBUG_UPDATE_SQL
		Verbose("%s: DNS UPDATE: %s", desctask(t), query);
#endif
	...
	if (!duplicate) {
		if (sql_nrquery(sql, query, querylen) != 0) {
			WarnSQL(sql, "%s: %s", desctask(t),
					_("error adding RR via DNS UPDATE"));
			return dnserror(t, DNS_RCODE_SERVFAIL, ERR_DB_ERROR);
		}
		sql_free(res);
	}
	...
	snprintf(query, sizeof(query), "ADD %s %u IN %s %u %s", rr->name, rr->ttl,
			mydns_qtype_str(rr->type), aux, data);
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on failure.
 **************************************************************************************************/
static int update_delete_rrset_all(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	...
	if (!(xhost = calloc(strlen(rr->name) * 2 + 1, sizeof(char))))
		Err(_("out of memory"));
	querylen = snprintf(query, sizeof(query), "%.*s",
			(int) (strlen(rr->name) - strlen(soa->origin) - 1), rr->name);
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on failure.
 **************************************************************************************************/
static int update_delete_rr(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	static char data[DNS_MAXPACKETLEN_UDP];
	uint32_t aux;
	...
	if (sql_nrquery(sql, query, querylen) != 0) {
		WarnSQL(sql, "%s: %s", desctask(t),
				_("error deleting RR via DNS UPDATE"));
		return dnserror(t, DNS_RCODE_SERVFAIL, ERR_DB_ERROR);
	}
	sql_free(res);

	/* Output info to verbose log */
	snprintf(query, sizeof(query), "DELETE %s IN %s %s", rr->name,
			mydns_qtype_str(rr->type), data);
	task_output_info(t, query);
	t->update_done++;
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on failure.
 **************************************************************************************************/
static int update_delete_rrset(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
	char *xname = NULL, *xhost = NULL;
	char query[512];
	size_t querylen;
	SQL_RES *res = NULL;
	...
	if (sql_nrquery(sql, query, querylen) != 0) {
		WarnSQL(sql, "%s: %s", desctask(t),
				_("error deleting RRset via DNS UPDATE"));
		return dnserror(t, DNS_RCODE_SERVFAIL, ERR_DB_ERROR);
	}
	sql_free(res);

	/* Output info to verbose log */
	snprintf(query, sizeof(query), "DELETE %s IN %s", rr->name,
			mydns_qtype_str(rr->type));
	task_output_info(t, query);
	t->update_done++;
...
/**************************************************************************************************
 ...
 Returns 0 on success, -1 on failure.
 **************************************************************************************************/
static int process_update(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr) {
#if DEBUG_ENABLED && DEBUG_UPDATE
 ...
 **************************************************************************************************/
static int check_tmprr(TASK *t, MYDNS_SOA *soa, UQ *q) {
	int n, i;
	...
	for (n = 0; n < q->num_tmprr; n++) {
		...
		if (mydns_rr_load(sql, &rr_first, soa->id, current_type, current_name,
				NULL) != 0) {
			sql_reopen();
			if (mydns_rr_load(sql, &rr_first, soa->id, current_type, current_name,
					NULL) != 0) {
		...
		for (i = 0; i < q->num_tmprr; i++)
			if (q->tmprr[i]->type == current_type
					&& !strcasecmp(q->tmprr[i]->name, current_name)) {
				q->tmprr[i]->checked = 1;
				total_prereq_rr++;
			}
#if DEBUG_ENABLED && DEBUG_UPDATE
			...
			if (q->tmprr[i]->type == current_type
				...
				for (rr = rr_first; rr && !found_match; rr = rr->next) {
					...
					if (!strcasecmp(rr->data, q->tmprr[i]->data)) {
						if (current_type == DNS_QTYPE_MX
								|| current_type == DNS_QTYPE_SRV) {
...
int dns_update(TASK *t) {
	...
	dns_update_error: build_reply(t, 1, sign_reply);
#if DEBUG_ENABLED && DEBUG_UPDATE
