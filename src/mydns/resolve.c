 ...
 **************************************************************************************************/
static int resolve_soa(TASK *t, datasection_t section, char *fqdn) {
	...
	return (
			section == ANSWER ? dnserror(t, DNS_RCODE_REFUSED, ERR_ZONE_NOT_FOUND) :
					0);
}
/*--- resolve_soa() -----------------------------------------------------------------------------*/
 ...
 **************************************************************************************************/
static int cname_recurse(TASK *t, datasection_t section, dns_qtype_t qtype,
	...
	for (n = 0; n < level; n++)
		if (t->Cnames[n] == cname->id) {
			...
			Verbose("%s: %s: %s %s %s (depth %d)", desctask(t),
					_("CNAME loop detected"), cname->name,
					mydns_qtype_str(cname->type), cname->data, level);
			return (1);
		}
 ...
 **************************************************************************************************/
static int process_rr(TASK *t, datasection_t section, dns_qtype_t qtype,
		char *fqdn, MYDNS_SOA *soa, char *label, MYDNS_RR *rr, int level) {
	register MYDNS_RR *r;
	register int rv = 0;
	register int add_ns = (section == ANSWER && !t->ns.size
			&& qtype != DNS_QTYPE_NS && qtype != DNS_QTYPE_ANY);
 ...
 **************************************************************************************************/
static inline void add_authority_ns(TASK *t, datasection_t section,
		MYDNS_SOA *soa, char *match_label) {
	if (!t->ns.size && section == ANSWER) {
		...
		for (label = match_label; *label; label++) {
			if (label == match_label || *label == '.') {
				if (label[0] == '.' && label[1])
					label++; /* Advance past leading dot */
 ...
 **************************************************************************************************/
static int resolve_label(TASK *t, datasection_t section, dns_qtype_t qtype,
 ...
 **************************************************************************************************/
int resolve(TASK *t, datasection_t section, dns_qtype_t qtype, char *fqdn,
		int level) {
	char name[DNS_MAXNAMELEN + 1];
	...
	if ((qtype == DNS_QTYPE_ANY) && (section == ANSWER)
			&& !strcasecmp(fqdn, soa->origin)) {
		rrlist_add(t, section, DNS_RRTYPE_SOA, (void *) soa, soa->origin);
		t->sort_level++;
	}
	...
	for (label = name;; label++) {
		if (label == name || *label == '.') {
			...
			if ((rv = resolve_label(t, section, qtype, fqdn, soa, label,
					label == name, level)) != 0)
				break;
		}
		if (!*label)
