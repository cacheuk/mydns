...
void db_connect(void) {
	...
	if (!user)
		user = conf_get(&Conf, "mysql-user", NULL);
	if (!host)
		host = conf_get(&Conf, "mysql-host", NULL);
	if (!password)
		password = conf_get(&Conf, "mysql-password", NULL);
	if (!password)
		password = conf_get(&Conf, "mysql-pass", NULL);
 ...
 **************************************************************************************************/
inline void meter(unsigned long current, unsigned long total) {
	char m[80];
	int num, len;
	time_t now;
	...
	memset(m, '.', 63);
	m[0] = m[60] = '|';
	m[5] = m[10] = m[15] = m[20] = m[25] = m[30] = m[35] = m[40] = m[45] =
			m[50] = m[55] = ':';
	m[61] = '\r';
	m[62] = '\0';
