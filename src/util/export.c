...
static void cmdline(int argc, char **argv) {
	char *optstr;
	int optc, optindex, n;
	struct option const longopts[] = { { "bind", no_argument, NULL, 'b' }, {
			"database", required_argument, NULL, 'D' }, { "host",
			required_argument, NULL, 'h' }, { "password", optional_argument, NULL,
			'p' }, { "tinydns-data", no_argument, NULL, 't' }, { "user",
	...
	for (n = 0; n < argc; n++) {
		if (!command_line) {
			...
			if (!(command_line = realloc(command_line,
					strlen(command_line) + strlen(argv[n]) + 2)))
				Err("realloc");
			strcat(command_line, " ");
 ...
 **************************************************************************************************/
static void bind_dump_soa(MYDNS_SOA *soa) {
	time_t now = time(NULL);
 ...
 **************************************************************************************************/
static void bind_dump_rr(MYDNS_SOA *soa, MYDNS_RR *rr, int maxlen) {
	...
	printf("\t%u\tIN %-5s\t", (rr->ttl < soa->minimum) ? soa->minimum : rr->ttl,
			type);

	if (rr->type == DNS_QTYPE_MX)
		printf("%u %s\n", (uint32_t) rr->aux, rr->data);
	else if (rr->type == DNS_QTYPE_SRV)
		printf("%u %u %u %s\n", (uint32_t) rr->aux, rr->srv_weight, rr->srv_port,
				rr->data);
...
/**************************************************************************************************
 TINYDNS_DUMP_SOA
 **************************************************************************************************/
static void tinydns_dump_soa(MYDNS_SOA *soa) {
	...
	printf("Z%s:%s:%s:%u:%u:%u:%u:%u:%u\n", origin, ns, mbox, soa->serial,
			soa->refresh, soa->retry, soa->expire, soa->minimum, soa->ttl);
}
/*--- tinydns_dump_soa() ------------------------------------------------------------------------*/
...
/**************************************************************************************************
 ...
 Output resource record, BIND format.
 **************************************************************************************************/
static void tinydns_dump_rr(MYDNS_SOA *soa, MYDNS_RR *rr, int maxlen) {
	...
	case DNS_QTYPE_SRV:
		strncpy(data, rr->data, sizeof(data));
		TINYDNS_NAMEFIX(data);
		printf("S%s::%s:%u:%u:%u:%u\n", name, data, rr->srv_port, rr->srv_weight,
				rr->aux, rr->ttl);
		break;
...
/**************************************************************************************************
 DUMP_SOA
 **************************************************************************************************/
static MYDNS_SOA *dump_soa(void) {
	MYDNS_SOA *soa;
...
/**************************************************************************************************
 DUMP_RR
 **************************************************************************************************/
static void dump_rr(MYDNS_SOA *soa, MYDNS_RR *rr, int maxlen) {
	switch (output_format) {
	case OUTPUT_BIND:
...
/**************************************************************************************************
 DUMP_RR_LONG
 **************************************************************************************************/
static void dump_rr_long(MYDNS_SOA *soa) {
	int maxlen = 0;
	char query[BUFSIZ];
	size_t querylen;
	SQL_RES *res;
	...
	if (!sql_count(sql, "SELECT COUNT(*) FROM %s WHERE zone=%u",
	...
	if (output_format == OUTPUT_BIND)
		maxlen = sql_count(sql, "SELECT MAX(LENGTH(name)) FROM %s WHERE zone=%u",
				mydns_rr_table_name, soa->id) + 1;
...
/**************************************************************************************************
 DUMP_ZONE
 **************************************************************************************************/
static void dump_zone(char *zone_name) {
	MYDNS_SOA *soa;
