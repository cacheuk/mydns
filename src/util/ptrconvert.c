...
#endif
		puts(
				_("  -v, --verbose           be more verbose while running (on by default)"));
 ...
 **************************************************************************************************/
static void load_zone_list(void) {
	SQL_RES *res;
	SQL_ROW row;
	char query[512];
	size_t querylen;
	...
	while ((row = sql_getrow(res))) {
		...
		/* Generate zone name */
		snprintf(name, sizeof(name), "%u.%u.%u.in-addr.arpa.", quad[2], quad[1],
				quad[0]);
		...
		if (n == numZones) {
			...
			if (!(Zones[numZones] = malloc(sizeof(ARPAZONE))))
				Err("malloc");
			strncpy(Zones[numZones]->name, name,
					sizeof(Zones[numZones]->name) - 1);
			Zones[numZones]->id = 0;
	...
	Verbose("loaded %d distinct zone%s from \"%s\" table", numZones, S(numZones),
...
void get_zone_ids(void) {
	...
	for (n = 0; n < numZones; n++) {
		...
		} else /* Insert new record */
		{
			...
			if ((id = sql_count(sql, "SELECT id FROM soa WHERE origin='%s'",
					esc_origin)) < 0)
...
void ptrconvert(void) {
	...
	for (n = 0; n < numZones; n++) {
		...
		while ((row = sql_getrow(res))) {
			...
			rv = sql_count(sql, "SELECT COUNT(*) FROM %s"
					" WHERE zone=%u AND type='PTR' AND (name='%u' OR name='%u.%s')",
					mydns_rr_table_name, Zones[n]->id, quad[3], quad[3],
					Zones[n]->name);
			...
			if (rv == 0) {
				...
				querylen =
						snprintf(query, sizeof(query),
								...
								mydns_rr_table_name, Zones[n]->id, quad[3], esc_data,
								ptr->ttl);
				if (sql_nrquery(sql, query, querylen) != 0)
...
int main(int argc, char **argv) {
	...
	Notice("processed %d resource record%s (%d found, %d created)",
			rr_found + rr_inserted, S(rr_found + rr_inserted), rr_found,
			rr_inserted);
