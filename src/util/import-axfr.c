...
extern uint32_t import_soa(const char *import_origin, const char *ns,
		const char *mbox, unsigned serial, unsigned refresh, unsigned retry,
		unsigned expire, unsigned minimum, unsigned ttl);
 ...
 **************************************************************************************************/
static int axfr_connect(char *hostportp, char **hostnamep) {
	char hostport[512];
 ...
 **************************************************************************************************/
char *make_question(uint16_t id, dns_qtype_t qtype, char *name, size_t *packetlen) {
	...
	/* ID */
	header.rd = 1;
	memcpy(dest, &header, sizeof(DNS_HEADER));
	dest += SIZE16;
	DNS_PUT16(dest, 1);
	/* QDCOUNT */
	DNS_PUT16(dest, 0);
 ...
 **************************************************************************************************/
static void request_axfr(int fd, char *rem_hostname, char *zone) {
	char *qb, *q, *p;
	size_t qlen;
	int rv, off = 0;
 ...
 **************************************************************************************************/
static void process_axfr_soa(char *name, char *reply, size_t replylen,
		char *src, uint32_t ttl) {
	...
	got_soa = import_soa(origin, ns, mbox, serial, refresh, retry, expire,
			minimum, ttl);
}
/*--- process_axfr_soa() ------------------------------------------------------------------------*/
 ...
 **************************************************************************************************/
static char *shortname(char *name, int empty_name_is_ok) {
...
/**************************************************************************************************
 ...
 is returned.
 **************************************************************************************************/
static char *process_axfr_answer(char *reply, size_t replylen, char *src) {
	...
	if (!(src = name_unencode(reply, replylen, src, name, sizeof(name))))
		Errx("%s: %s: %s", hostname, _("error reading name from answer section"),
				name);
	...
	case DNS_QTYPE_HINFO: {
		...
		snprintf(insdata, sizeof(insdata), "%s%s%s %s%s%s", quote1 ? "\"" : "",
				data, quote1 ? "\"" : "", quote2 ? "\"" : "", data2,
				quote2 ? "\"" : "");
	...
	case DNS_QTYPE_RP: {
		...
		snprintf(insdata, sizeof(insdata), "%s %s", shortname(data, 0),
				shortname(txtref, 0));
	...
	case DNS_QTYPE_SRV: {
		...
		snprintf(databuf, sizeof(databuf), "%u %u %s", weight, port,
				shortname(data, 0));
	...
	default:
		Warnx("%s %s: %s", name, mydns_qtype_str(type),
				_("discarding unsupported RR type"));
		break;
	}
	return (rv);
}
...
/**************************************************************************************************
 PROCESS_AXFR_REPLY
 **************************************************************************************************/
static int process_axfr_reply(char *reply, size_t replylen) {
	...
	if (hdr.rcode != DNS_RCODE_NOERROR)
		Errx("%s: %s: %s", hostname,
				_("server responded to our request with error"),
				mydns_rcode_str(hdr.rcode));

#if DEBUG_ENABLED
	Debug("%zu byte REPLY: qr=%u opcode=%s aa=%u tc=%u rd=%u ra=%u z=%u rcode=%u qd=%u an=%u",
	...
	for (n = 0; n < qdcount; n++) {
		if (!(src = name_unencode(reply, replylen, src, name, sizeof(name))))
			Errx("%s: %s: %s", hostname,
					_("error reading name from question section"), name);
		src += (SIZE16 * 2);
	}
...
/**************************************************************************************************
 IMPORT_AXFR
 **************************************************************************************************/
void import_axfr(char *hostport, char *import_zone) {
	char *reply, len[2];
	int fd;
	size_t replylen;
