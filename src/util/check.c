...
static void cmdline(int argc, char **argv) {
	char *optstr;
	int optc, optindex;
	struct option const longopts[] = { { "consistency-only", no_argument, NULL,
			'C' }, { "consistency", no_argument, NULL, 'c' },
#ifdef EXTENDED_CHECK_WRITTEN
...
static void rrproblem(const char *fmt, ...) {
	va_list ap;
...
/**************************************************************************************************
 CHECK_NAME_EXTENDED
 **************************************************************************************************/
static void check_name_extended(const char *name_in, const char *fqdn,
		const char *col) {
 ...
 **************************************************************************************************/
static char *shortname(char *name_to_shorten, int empty_name_is_ok) {
 ...
 **************************************************************************************************/
static void check_name(const char *name_in, const char *col, int is_rr) {
			...
			if (strcmp(label, "*")) {
				...
				for (cp = label; *cp; cp++) {
					...
					if (!isalnum((int)(*cp)) && *cp != '-') {
						if (is_rr && *cp == '*')
							rrproblem(_("Wildcard character `%c' in `%s' not alone"),
									*cp, col);
						else
							rrproblem(
									_("Label in `%s' contains illegal character `%c'"),
									col, *cp);
					}
				}
			} else if (!is_rr)
 ...
 **************************************************************************************************/
static MYDNS_SOA *check_soa(const char *zone) {
	...
	*name = *data = '\0';
	...
	if (soa->refresh < 300)
		rrproblem(_("soa.refresh is less than 300 seconds"));
	if (soa->retry < 300)
		rrproblem(_("soa.retry is less than 300 seconds"));
	if (soa->expire < 300)
		rrproblem(_("soa.expire is less than 300 seconds"));
	if (soa->minimum < 300)
		rrproblem(_("soa.minimum is less than 300 seconds"));
	if (soa->ttl < 300)
		rrproblem(_("soa.ttl is less than 300 seconds"));
	if (soa->minimum < 300)
		rrproblem(_("soa.minimum is less than 300 seconds"));
 ...
 **************************************************************************************************/
static void check_rr_cname(void) {
	char *xname;
	int found = 0;
	...
	found =
			sql_count(sql,
					"SELECT COUNT(*) FROM %s WHERE zone=%u AND name='%s' AND type != 'CNAME'",
 ...
 **************************************************************************************************/
static void check_rr_hinfo(void) {
 ...
 **************************************************************************************************/
static void check_rr_naptr(void) {
...
static void check_zone(void) {
	...
	querylen = snprintf(query, sizeof(query),
			"SELECT "MYDNS_RR_FIELDS" FROM %s WHERE zone=%u", mydns_rr_table_name,
			soa->id);
	...
	if (err_verbose) {
		meter(0, 0);
		Verbose("%s: %u %s", soa->origin, rrct,
				rrct == 1 ? _("resource record") : _("resource records"));
	}
}
/*--- check_zone() ------------------------------------------------------------------------------*/
 ...
 **************************************************************************************************/
static void consistency_rr_zone(void) {
	char query[BUFSIZ];
	size_t querylen;
	SQL_RES *res;
	SQL_ROW row;
	...
	while ((row = sql_getrow(res))) {
		...
		meter(0, 0);
		snprintf(msg, sizeof(msg), _("%s id %s references invalid %s id %s"),
...
int main(int argc, char **argv) {
	...
	if (!opt_consistency_only) {
		if (optind >= argc) /* Check all zones */
		{
			...
			querylen = snprintf(query, sizeof(query), "SELECT origin FROM %s",
					mydns_soa_table_name);
			if ((res = sql_query(sql, query, querylen))) {
			...
			while (optind < argc) /* Check zones provided as args */
			{
	...
	if (!syntax_errors && !consistency_errors)
		...
		if (opt_consistency_only)
			...
			Verbose("%s: %d  %s: %d", _("Syntax errors"), syntax_errors,
					_("Consistency errors"), consistency_errors);
		else
