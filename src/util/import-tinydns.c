...
extern void import_rr(char *name, char *type, char *data, unsigned aux,
		unsigned ttl);
 ...
 **************************************************************************************************/
static void twarn(const char *fmt, ...) {
	va_list ap;
	char buf[1024];
 ...
 **************************************************************************************************/
static inline int zone_ok(const char *fqdn) {
	...
	if (strlen(fqdn) > strlen(tinydns_zone)
			&& !strcasecmp(fqdn + strlen(fqdn) - strlen(tinydns_zone),
					tinydns_zone))
		return 1;
 ...
 **************************************************************************************************/
static int zonecmp(const void *p1, const void *p2) {
 ...
 **************************************************************************************************/
static inline ZONE *find_zone(const char *origin) {
	register int n;
 ...
 **************************************************************************************************/
static inline ZONE *find_host_zone(char *fqdn, char *hostname) {
	register char *c;
	ZONE *z;
...
/**************************************************************************************************
 ...
 NULL if not found.
 **************************************************************************************************/
static inline ZONE *find_arpa_zone(const char *ip, char *hostname) {
 ...
 **************************************************************************************************/
static ZONE *new_zone(const char *originp) {
	ZONE *z;
	char origin[DNS_MAXNAMELEN + 1];

	/* If this is an 'in-addr.arpa' zone, knock off the first quad if present */
	if (strlen(originp) > 12
			&& !strcasecmp(originp + strlen(originp) - 13, ".in-addr.arpa")) {
 ...
 **************************************************************************************************/
void tinydns_add_soa(char *line) {
	char *l = line + 1;
	ZONE *z;

	if (line[0] == '.') /* '.' (NS) line */
	{
		char *fqdn = NULL, *ip = NULL, *x = NULL, *ttl = NULL, *timestamp = NULL,
				*lo = NULL;
		...
		if ((x = strsep(&l, ":")) && (ttl = strsep(&l, ":")) && (timestamp =
				strsep(&l, ":")) && (lo = strsep(&l, ":")))
			/* DONOTHING */;
	...
	{
		char *fqdn = NULL, *ns = NULL, *mbox = NULL, *serial = NULL, *refresh =
				NULL, *retry = NULL, *expire = NULL, *minimum = NULL, *ttl = NULL,
				*timestamp = NULL, *lo = NULL;
		...
		if ((serial = strsep(&l, ":")) && (refresh = strsep(&l, ":")) && (retry =
				strsep(&l, ":")) && (expire = strsep(&l, ":"))
				&& (minimum = strsep(&l, ":")) && (ttl = strsep(&l, ":"))
		...
		if (serial)
			z->serial = atol(serial);
		if (refresh)
			z->refresh = atol(refresh);
		if (retry)
			z->retry = atol(retry);
		if (expire)
			z->expire = atol(expire);
		if (minimum)
			z->minimum = atol(minimum);
		if (ttl)
			z->ttl = atol(ttl);
	}
}
 ...
 **************************************************************************************************/
static void create_zone(ZONE *z) {
	char origin[DNS_MAXNAMELEN + 3], ns[DNS_MAXNAMELEN + 3], mbox[DNS_MAXNAMELEN
			+ 3];
	...
	z->id = import_soa(origin, ns, mbox, z->serial, z->refresh, z->retry,
			z->expire, z->minimum, z->ttl);
}
/*--- create_zone() -----------------------------------------------------------------------------*/
...
static void tinydns_plus(void) {
	...
	if (!(z = find_host_zone(fqdn, hostname)))
		return (void) Warnx("%s:%u: %s: %s", filename, lineno, fqdn,
				_("fqdn does not match any zones"));
 ...
 **************************************************************************************************/
static void tinydns_equal(void) {
	ZONE *z;
	char *fqdn = field[0], *ip = field[1], *ttl = field[2];
	char hostname[DNS_MAXNAMELEN + 1] = "";
	...
	if ((z = find_arpa_zone(ip, hostname))) {
		...
		if (!(a = strsep(&p, ".")) || !(b = strsep(&p, "."))
				|| !(c = strsep(&p, ".")) || !(d = strsep(&p, ".")))
			return;
		Warnx("%s:%u: %s.%s.%s.%s.in-addr.arpa: %s", filename, lineno, d, c, b, a,
				_("no matching zone for PTR record"));
	}
}
/*--- tinydns_equal() ---------------------------------------------------------------------------*/
 ...
 **************************************************************************************************/
static void tinydns_at(void) {
	ZONE *z;
	char *fqdn = field[0], *ip = field[1], *x = field[2], *dist = field[3],
			*ttl = field[4];
	...
	if (!x || !strlen(x))
		return (void) Warnx("%s:%u: %s", filename, lineno,
				_("no mail exchanger specified"));
	...
	if (LASTCHAR(mx) != '.')
		strcat(mx, ".");
	import_rr(hostname, "MX", mx, dist ? atol(dist) : 0,
			ttl ? atol(ttl) : z->ttl);
 ...
 **************************************************************************************************/
static void tinydns_caret(void) {
	ZONE *z;
 ...
 **************************************************************************************************/
static void tinydns_colon(void) {
	ZONE *z;
	...
	Warnx("%s:%u: %s: %s", filename, lineno, fqdn,
			_("generic record type not supported"));
}
/*--- tinydns_colon() ---------------------------------------------------------------------------*/
 ...
 **************************************************************************************************/
void import_tinydns(char *datafile, char *import_zone) {
	struct stat st;
	FILE *fp;
	char buf[4096], *b;
	int n;
	...
	while (fgets(buf, sizeof(buf), fp)) {
		...
		default:
			Warnx("%s:%u: %s (0x%X)", filename, lineno, _("unknown line type"),
					buf[0]);
			break;
		}
	}
